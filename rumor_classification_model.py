...
def train(_args):
    #训练函数
    @tf.function
    def train_step(content_seq, content_mask, x_label,comment_seq,comment_mask):
        with tf.GradientTape() as tape:
            ...
            loss = loss_obj(x_label, outputs)

        grads = tape.gradient(loss,trainable_weights)

        opt.apply_gradients(zip(grads, trainable_weights))
        acc_item = tf.equal (tf.argmax(outputs, axis=-1, output_type=x_label.dtype), x_label)
        acc = tf.reduce_sum(tf.cast(acc_item, dtype=tf.float32)) / tf.cast(tf.shape(acc_item)[0], tf.float32)
        return loss, acc,tf.nn.sigmoid(outputs)


    @tf.function
    def eval_step(content_seq, content_mask, x_label,comment_seq,comment_mask):
        encoder_logits = rumor_encoder_model(content_seq, content_mask, training=True)
        emotion_logits = tf.stop_gradient(emotion_backbone_model(comment_seq,training=False))
        outputs = merge_model([encoder_logits,emotion_logits])
        loss = loss_obj(x_label, outputs)
        acc_item = tf.equal(tf.argmax(outputs, axis=-1, output_type=x_label.dtype), x_label)
        acc = tf.reduce_sum(tf.cast(acc_item, dtype=tf.float32)) / tf.cast(tf.shape(acc_item)[0], tf.float32)
        pred_labels = tf.math.argmax(outputs, axis=-1)  # 获取预测标签
        return loss, acc, tf.nn.sigmoid(outputs)  # 返回预测标签
    ...
    trainable_weights = rumor_encoder_model.trainable_weights + merge_model.trainable_weights
    ...
    if _args.enable_discriminative_lr:
        ...
        opt = MultiOptimizer([(opt_emb,rumor_encoder_model.layers[0]),(opt_other,rumor_encoder_model.layers[1:] + merge_model.layers)])
    else:
    ...
    train_precision = Precision(name="准确率")
    train_recall = Recall(name="召回率")
    # train_f1score = F1Scare
    ...
    for n in range(0,_args.train_epochs):
        ...
        content_train, label_train, content_mask_train, comment_train, comment_mask_train = get_rumor_classification_dataset(
            _args.train_filepath, _args.min_sequence_length, _args.sequence_length, _args.n_comment_keeps,tokenizer_type=_args.used_embedding)
        content_test, label_test, content_mask_test, comment_test, comment_mask_test = get_rumor_classification_dataset(
            _args.test_filepath, _args.min_sequence_length, _args.sequence_length, _args.n_comment_keeps,tokenizer_type=_args.used_embedding)
        n_train_total_steps = len(comment_train) // _args.batch_size
        n_eval_total_steps = len(comment_train) // _args.batch_size
        ...
        with tqdm(total=n_train_total_steps) as bar:
            for i in range(n_train_total_steps):
                ...
                #训练部分
                train_batch_loss, train_bach_acc, predict_labels = train_step(batch_content_train,batch_content_mask_train,batch_label_train, batch_comment_train,batch_comment_mask_train)
                train_precision.update_state(y_true=batch_label_train, y_pred=predict_labels)
                ...
                bar.update()
        train_precision_result = train_precision.result()
        train_recall_result = train_recall.result()

        train_f1score_result = 2 * (train_precision_result * train_recall_result) / (
                train_precision_result + train_recall_result)
        ...
        eval_precision.reset_state()
        eval_recall.reset_state()
        ...
        with tqdm(total=n_eval_total_steps) as bar:
            ...
            for i in range(n_eval_total_steps):
                ...
                batch_comment_test = comment_test[i * _args.batch_size: (i + 1) * _args.batch_size]
                batch_comment_mask_test = comment_mask_test[i * _args.batch_size: (i + 1) * _args.batch_size]
                eval_batch_loss, eval_batch_acc,eval_outputs = eval_step(
                    batch_content_test, batch_content_mask_test,
                    batch_label_test, batch_comment_test, batch_comment_mask_test)
                eval_loss_sum += float(eval_batch_loss)
                eval_acc_sum += float(eval_batch_acc)
                ...
                bar.set_postfix({'epoch': n + 1,
                                 ...
                                 'eval_acc': round(float(eval_batch_acc), 4)})
                bar.update()
                eval_precision.update_state(batch_label_test, eval_outputs)
                eval_recall.update_state(batch_label_test, eval_outputs)
            ...
            eval_precision_result = eval_precision.result()
            eval_recall_result = eval_recall.result()

            eval_f1score_result = 2 * (eval_precision_result * eval_recall_result) / (
                        eval_precision_result + eval_recall_result)
            eval_loss.append(eval_loss_avg)
            eval_acc.append(eval_acc_avg)
            print(f'epoch: {n+1}/{_args.train_epochs},avg_eval_loss:{eval_loss_avg},avg_eval_acc: {eval_acc_avg},精确率: {eval_precision.result()}, 查全率: {eval_recall.result()}, F1Score: {eval_f1score_result}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', default=1, type=int,help='设置批次大小')
    parser.add_argument('--n-heads', default=4, type=int,help='设置多头注意力的头的个数')
    parser.add_argument('--train-epochs', default=6, type=int,help='设置训练的次数')
